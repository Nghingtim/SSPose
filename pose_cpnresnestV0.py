...
class PoseHGResNeSt(nn.Module):
    def __init__(self, cfg, pretrained=True, num_keypoints=17):
        ...
        self.splitAttention = SplAtConv2d(
                in_channels=2048, channels=256, kernel_size=3,
                stride=1, padding=1,
                dilation=1, groups=1, bias=False,
                radix=8, rectify=False,
                rectify_avg=False,
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            ...
            logger.info('CPNResNeStV0=> init lateral2 conv weights from normal distribution')
            for   m in self.lateral1.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
 
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.lateral2.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
 
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.lateral3.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
 
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.lateral4.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            ...
            logger.info('=> init smooth1 conv weights from normal distribution')
            for m in self.smooth1.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
 
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.smooth2.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
 
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            for m in self.smooth3.modules():
                if isinstance(m, nn.Conv2d):
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            ...
            logger.info('=> init global1 weights from normal distribution')

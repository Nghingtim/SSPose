...
class PoseHGResNeSt(nn.Module):
    def __init__(self, cfg, pretrained=True):
        ...
        self.downConv1 = self._make_downConv(256, 256, 1)
        self.downConv2 = self._make_downConv(512, 512, 1)
        self.downConv3 = self._make_downConv(1024, 1024, 1)
    ...
    def forward(self, x): 
        ...
        y3 = self.deconv2(self.relu(y2))  #32*32
        #print(y3.shape)
        
        y4 = y3 + self.downConv2(x2)

        y5 = self.deconv3(self.relu(y4)) #64*64
        #print(y5.shape)
        
        y6 = y5 + self.downConv1(x1)

        x = self.final_layer(self.relu(y6))
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            logger.info('V4: PoseHGResNeSt50=> init deconv weights from normal distribution')

...
import sys
sys.path.insert(0, '/home/cair2016/Projects/deep-high-resolution-net.pytorch/lib/models/Res2Net/')
...
class PoseRes2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        y3 = self.deconv2(self.relu(y2))
        #print(y3.shape)
        
        y4 = y3 + self.downConv2(x2)

        y5 = self.deconv3(self.relu(y4))
        #print(y5.shape)
        ...
        x = self.final_layer(self.relu(x))  #16 64 64
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            logger.info(' Res2Net50V3=> init deconv weights from normal distribution')
            ...
            logger.info(' Res2Net50V3=> init downconv weights from normal distribution')
            for name, m in self.downConv1.named_modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    nn.init.constant_(m.weight, 1)
                    nn.init.constant_(m.bias, 0)
            for name, m in self.downConv2.named_modules():
                if isinstance(m, nn.Conv2d):
                    logger.info('=> init {}.weight as normal(0, 0.001)'.format(name))
                    logger.info('=> init {}.bias as 0'.format(name))
                    nn.init.normal_(m.weight, std=0.001)
                    ...
                    nn.init.constant_(m.weight, 1)
                    nn.init.constant_(m.bias, 0)
            for name, m in self.downConv3.named_modules():
                if isinstance(m, nn.Conv2d):
                    logger.info('=> init {}.weight as normal(0, 0.001)'.format(name))
                    logger.info('=> init {}.bias as 0'.format(name))
                    nn.init.normal_(m.weight, std=0.001)
